...
public final class ReplantQueue {
    ...
    public synchronized void start() {
        if (started) return;
        started = true;
        scheduledTask = plugin.getServer().getGlobalRegionScheduler().runAtFixedRate(plugin, (task) -> this.tick(), 1L, 1L);
    }
    ...
    public synchronized void enqueue(
            Block block, Material plantMaterial, int delayTicks,
            int targetAge, BlockFace cocoaFacingDirection) {
        ...
        while (head != -1) {
            ...
            if (loaded) {
                    ...
                    if (plant != null) {
                        ...
                        if (info == null) {
                            WarningThrottle.log(plugin, Level.WARNING,
                                    ...
                                    "No age data found for plant: " + plant + ", skipping replant at " + locString(b));
                ...
                if (retries >= MAX_UNLOAD_RETRIES) {
                    WarningThrottle.log(plugin, Level.WARNING, WarningThrottle.CAT_ABANDONED,
    ...
    private void replantNormal(int index, AgeMetaRegistry.CropInfo info) {
        ...
        block.setBlockData(info.getBlockData(targetAge), false);
    }
    ...
    private void replantCocoa(int index, AgeMetaRegistry.CropInfo info) {
        ...
        if (!CropConstants.JUNGLE_ANCHOR_BLOCKS.contains(attached.getType())) return;

        block.setBlockData(info.getCocoaBlockData(targetAge, faceOrd), false);
    }
